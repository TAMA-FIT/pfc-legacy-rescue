...
c.innerHTML=`<div style="font-size:12px;font-weight:900;color:#187a51">PFC LEGACY RESCUE</div><h2 style="margin:5px 0 12px">旧データを検出しました</h2><div style="display:grid;grid-template-columns:1fr auto;gap:8px 14px;padding:12px;background:#f1faf5;border-radius:14px"><span>主要PFC項目</span><b>${core}/8</b><span>追加保存キー</span><b>${Object.keys(raw).length}</b><span>体型写真</span><b>${pr.photos.length}枚</b><span>JSONサイズ</span><b>${(text.length/1048576).toFixed(2)}MB</b></div><p style="font-size:13px;line-height:1.6;color:#5f7168">旧データは変更していません。保存ボタンで端末の共有/保存画面を開きます。</p>`;
...
x.onclick=()=>{ov.remove();window.__PFC_LEGACY_RESCUE_RUNNING__=false};c.append(b,x,s);ov.append(c);document.documentElement.append(ov);
if(core===0&&!Object.keys(raw).length&&!pr.photos.length)s.textContent='PFCデータが見つかりません。同じSafari/ブラウザプロフィールか確認してください。';
